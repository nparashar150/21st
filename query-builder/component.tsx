        ...
        <PopoverContent className="w-32 p-1" align="start">
          <div className="max-h-48 overflow-y-auto">
            {options.length === 0 ? (
              ...
              options.map((option) => (
                <button key={option.value} onClick={() => handleSelect(option.value)} className={cn("hover:bg-accent flex w-full items-center rounded-sm px-2 py-1.5 text-sm", value.includes(option.value) && "bg-accent")}>
...
function FieldSelector({ fields, groupedFields, value, onChange, disabled }: { fields: Field[]; groupedFields: Map<string, Field[]>; value: string; onChange: (value: string) => void; disabled?: boolean }) {
  const [open, setOpen] = React.useState(false);
...
function RuleRow({ rule, fields, onUpdate, onRemove, disabled }: { rule: Rule; fields: Field[]; onUpdate: (updates: Partial<Rule>) => void; onRemove: () => void; disabled?: boolean }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn("group/rule flex items-center gap-2 transition-all duration-200", isDragging && "opacity-90", disabled && "opacity-50 pointer-events-none")}>
      ...
      <div className={cn("flex min-w-0 flex-1 items-center gap-2 rounded-lg border p-2.5", "bg-white/80 dark:bg-white/3 border-slate-200/60 dark:border-white/10", "hover:border-slate-300 dark:hover:border-primary/40", isDragging && "shadow-lg border-primary/50")}>
  ...
  const groupContent = (
    <div
      className={cn(
        ...
        isRoot 
          ? "border-slate-200/60 dark:border-white/10 bg-white/50 dark:bg-white/3" 
          : "border-violet-200 dark:border-primary/20 bg-violet-50/50 dark:bg-primary/5",
        isDragging && "shadow-lg border-primary/50",
        disabled && "opacity-50 pointer-events-none"
      )}
    >
      <div className="flex items-center gap-2 p-3">
        <div className="border-slate-200 dark:border-white/20 inline-flex h-7 items-center overflow-hidden rounded-md border text-xs">
          <button
          ...
          <div className="bg-slate-200 dark:bg-white/20 h-full w-px" />
          <button
      ...
      <div className="space-y-2 p-3 pt-0">
        {group.rules.length === 0 ? (
          <div className="border-slate-200 dark:border-white/20 flex flex-col items-center justify-center rounded-lg border border-dashed py-8">
                  ...
                  <React.Fragment key={item.id}>
                    {index > 0 && (
                      <div className="flex items-center gap-2 py-1">
                        <div className="bg-slate-200 dark:bg-white/20 h-px flex-1" />
                    ...
                    {item.type === "rule" ? (
                      <RuleRow
                        rule={item}
                        fields={fields}
                        onUpdate={(updates) => onChange({ ...group, rules: group.rules.map((r) => (r.id === item.id && r.type === "rule" ? { ...r, ...updates } : r)) })}
                        onRemove={() => onChange({ ...group, rules: group.rules.filter((r) => r.id !== item.id) })}
                        disabled={disabled}
                      />
                    ) : (
                      <RuleGroupComponent
                        group={item}
                        fields={fields}
                        onChange={(newGroup) => onChange({ ...group, rules: group.rules.map((r) => (r.id === item.id ? newGroup : r)) })}
                        onRemove={() => onChange({ ...group, rules: group.rules.filter((r) => r.id !== item.id) })}
                        depth={depth + 1}
                        disabled={disabled}
                      />
